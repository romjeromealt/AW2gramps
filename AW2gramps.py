...
def nettoyer_html(texte):
    ...
    texte = re.sub(r'<i>(.*?)</i>', r'/\1/', texte)
    texte = re.sub(r'<b>(.*?)</b>', r'*\1*', texte)
    texte = re.sub(r'<u>(.*?)</u>', r'_\1_', texte)
    texte = re.sub(r'<a\s+[^>]*href=["\']([^"\']*)["\'][^>]*>(.*?)</a>', r'\2 (\1)', texte)
    texte = re.sub(r'<br\s*/?>', '\n', texte)
...
def ajouter_styles_texte(note_elem, note_text):
    ...
    titres_a_styliser = [
        "Historique du nom de la place", "Construction", "Description", "Galerie",
        "Événements", "Les maisons de l'Ecomusée", "La forêt des jeux"
    ]
...
def extract_gallery(text):
    """Extrait les fichiers d'une balise <gallery>."""
...
def extract_refs(text):
    """Extrait les balises <ref>...</ref> d'un texte."""
...
def process_media_links(note_text, media_handles):
    """Traite les liens vers les médias dans le texte."""
...
def parse_person_field(person_str):
    """
    Extrait et structure les personnes (architectes/entrepreneurs) à partir du champ CSV "Personne".
    """
    people = []
    if not person_str:
        return people

    person_blocks = [p.strip() for p in person_str.split('),') if p.strip()]
    for block in person_blocks:
        ...
        if len(parts) >= 1:
            person['name'] = parts[0].replace('Personne:', '').strip()
        if len(parts) >= 2:
            person['job'] = parts[1]
        if len(parts) >= 3:
            person['date_range'] = parts[2]
        if len(parts) >= 4:
            person['event_num'] = parts[3]
...
def csv_to_gramps_xml(csv_file_path, output_xml_file_path):
    ...
    for i, row in enumerate(data, 1):
        ...
        # Extraction des coordonnées et création des lieux
        ...
        for j, (lat, lon) in enumerate(coords_pairs, 1):
            ...
            # Mise à jour des événements avec le lieu
            for event in events_data:
                if not event['place_handle']:
                    event['place_handle'] = f"_{place_id}"

            place_id += 1

        # Extraction des personnes (architectes/entrepreneurs)
        people = parse_person_field(row.get('Personne', ''))
        for person in people:
            if person['name'] not in people_data:
                people_data[person['name']] = {
                    'handle': f"_{person_id}",
                    'surname': person['name'].split()[-1],
                    'firstname': ' '.join(person['name'].split()[:-1]),
                    'note_handles': set(note_refs),
                    'event_num': person.get('event_num'),
                }
                person_handles[person['name']] = f"_{person_id}"
                person_id += 1

        # Lien entre personnes et événements
        for person in people_data.values():
            for event in events_data:
                if event.get('event_num') == person.get('event_num'):
                    event['architects'].add(person['handle'])
    ...
    for architect, person in people_data.items():
        person_elem = ET.SubElement(people_elem, 'person',
            handle=person['handle'],
            change=str(int(datetime.now().timestamp()))
        )
        name = ET.SubElement(person_elem, 'name', type="Birth Name")
        ET.SubElement(name, 'surname').text = person['surname']
        ET.SubElement(name, 'first').text = person['firstname']

        for event in events_data:
            if person['handle'] in event.get('architects', set()):
                ET.SubElement(person_elem, 'eventref',
                    hlink=event['handle'],
                    role='Architect' if 'Architecte' in person.get('job', '') else 'Unknown'
                )

        for note_handle in person.get('note_handles', set()):
            ET.SubElement(person_elem, 'noteref', hlink=note_handle)

    # Création des éléments XML pour les lieux
    for place_key, place in places_data.items():
        place_elem = ET.SubElement(places_elem, 'placeobj',
            handle=place['handle'],
            change=str(int(datetime.now().timestamp()))
        )
        ET.SubElement(place_elem, 'ptitle').text = place['title']
        pname = ET.SubElement(place_elem, 'pname')
        pname.set('value', place['title'])

        if place['lat'] is not None and place['lon'] is not None:
            coord = ET.SubElement(place_elem, 'coord')
            coord.set('lat', f"{place['lat']:.6f}")
            coord.set('long', f"{place['lon']:.6f}")

        for note_handle in place.get('note_handles', set()):
            ET.SubElement(place_elem, 'noteref', hlink=note_handle)

    # Création des éléments XML pour les médias
    for media in media_handles.values():
        media_elem = ET.SubElement(objects_elem, 'object',
            handle=media['handle'],
            change=str(int(datetime.now().timestamp())),
            type='Media'
        )
    ...
    for note in notes_data:
        ...
        for media_handle in note['media_handles']:
            ET.SubElement(note_elem, 'objref', hlink=media_handle)

        for source_handle in note['source_handles']:
            ET.SubElement(note_elem, 'sourceref', hlink=source_handle)

        for obj_handle in note['object_handles']['people']:
            ET.SubElement(note_elem, 'objref', hlink=obj_handle)
        ...
        ET.SubElement(note_elem, 'text').text = note['text']
