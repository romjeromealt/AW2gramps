...
print("🔧 Configuration du prétraitement pour l'OCR")
print("--------------------------------------------")

# Paramètres pour le filtre bilatéral (texte normal)
d = int(input("Taille du filtre bilatéral [d] (5-15, défaut=9): ") or 9)
sigmaColor = int(input("Sensibilité couleur [sigmaColor] (50-100, défaut=75): ") or 75)

# Paramètres pour la binarisation adaptative (texte normal)
blockSize = int(input("Taille du bloc pour la binarisation [blockSize] (3-19, impair, défaut=11): ") or 11)
if blockSize % 2 == 0:
    blockSize += 1  # Forcer un nombre impair
C = int(input("Ajustement du seuil [C] (0-10, défaut=2): ") or 2)

# Paramètres pour la fermeture morphologique (texte normal)
iterations_close = int(input("Nombre d'itérations pour la fermeture [iterations] (1-3, défaut=1): ") or 1)

# Paramètres pour les colonnes de folios (nombres)
iterations_dilate = int(input("Nombre d'itérations pour la dilatation (folios) [iterations] (1-3, défaut=2): ") or 2)
...
def preprocess_image(img, is_digit_column=False):
    ...
    if is_digit_column:
        ...
        processed = cv2.dilate(thresh, kernel, iterations=iterations_dilate)
    else:
        # Pour le texte : filtre bilatéral + binarisation adaptative + fermeture
        blurred = cv2.bilateralFilter(gray, d=d, sigmaColor=sigmaColor, sigmaSpace=75)
        ...
        processed = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel, iterations=iterations_close)
...
col_ranges = [
    ...
    (col_width, 2 * col_width),    # Colonne 2 (Folios)
    (2 * col_width, 3 * col_width),# Colonne 3 (Nom/Prénom/Demeure)
    (3 * col_width, 4 * col_width),# Colonne 4 (Folios)
    (4 * col_width, 5 * col_width),# Colonne 5 (Nom/Prénom/Demeure)
    (5 * col_width, img_width)     # Colonne 6 (Folios)
]
...
df.to_csv("extracted_data_custom.csv", index=False, encoding="utf-8-sig")
print("✅ Données exportées vers 'extracted_data_custom.csv'")
